...
const OrderTable = ({ tableHeader, orders, getStatusColor, openAddMat ,openAddCons, tableType}) => {
                    ...
                    <tbody className="bg-white lg:border-gray-300">
                        {orders.slice((currentPage - 1) * ordersPerPage, Math.min(currentPage * ordersPerPage, orders.length)).map(order => (
                            <tr key={order.orderId} className="gap-5">
                                <td className="whitespace-no-wrap min-w-36  py-4 text-sm font-normal text-gray-600 sm:px-3 lg:table-cell">{order.orderDate}</td>
                                <td className="whitespace-no-wrap min-w-36 py-4 text-sm font-normal text-gray-600 sm:px-3 lg:table-cell">{order.orderId}</td>
                                <td className="whitespace-no-wrap min-w-36 py-4 text-sm font-normal text-gray-600 sm:px-3 lg:table-cell">{order.description}</td>
                                <td className="whitespace-no-wrap min-w-36 py-4 text-left text-sm text-gray-600 sm:px-3 lg:table-cell lg:text-left">{order.materialType}</td>
                                <td className="whitespace-no-wrap min-w-36 py-4 text-left text-sm text-gray-600 sm:px-3 lg:table-cell lg:text-left">{order.invNum}</td>
